...
FSM_State<T>::FSM_State(ControlFSMData<T>* _controlFSMData,
    ...
    : _data(_controlFSMData),
      ...
      _logger("FSM_State") {
  transitionData.zero();
  QUADRUPED_INFO(_logger, "Initialized FSM state: %s", stateStringIn.c_str());
}
...
#if (USE_RS485_A1 == 1)
  kpMat << 0.625, 0, 0, 0, 0.625, 0, 0, 0, 0.625;
  kdMat << 0.05, 0, 0, 0, 0.05, 0, 0, 0, 0.05;
#else
  kpMat << 90, 0, 0, 0, 90, 0, 0, 0, 90;
...
void FSM_State<T>::runControls() {
  ...
  if (CONTROLLER_OPTION == 0) {
    ...
    QUADRUPED_ERROR(_logger, "No known controller was selected: %d", CONTROLLER_OPTION);
  }
}
