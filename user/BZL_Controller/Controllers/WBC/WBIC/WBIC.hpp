...
class WBIC : public WBC<T> {
 ...
 private:
  ...
  size_t _dim_opt = 0;      // Contact pt delta, First task delta, reaction force
  size_t _dim_eq_cstr = 0;  // equality constraints

  size_t _dim_rf = 0;  // inequality constraints
  size_t _dim_Uf = 0;

  size_t _dim_floating;

  WBIC_ExtraData<T>* _data = nullptr;
