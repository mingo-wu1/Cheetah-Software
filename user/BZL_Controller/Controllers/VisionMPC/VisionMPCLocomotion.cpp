  ...
  _logger("VisionGait")
{
  ...
  QUADRUPED_INFO(_logger, 
    ...
    \tnMPC_segments   :[%d]\n\
    \toffsets(int)    :[%d %d %d %d]\n\
    \tdurations(int)  :[%d %d %d %d]\n\
    \toffsets(float)  :[%.1f %.1f %.1f %.1f]\n\
    \tdurations(float):[%.1f %.1f %.1f %.1f]",
    name.c_str(),
    _nIterations,
    _offsets.transpose()(0), _offsets.transpose()(1), _offsets.transpose()(2), _offsets.transpose()(3),
    _durations.transpose()(0), _durations.transpose()(1), _durations.transpose()(2), _durations.transpose()(3),
    _offsetsFloat.transpose()(0), _offsetsFloat.transpose()(1), _offsetsFloat.transpose()(2), _offsetsFloat.transpose()(3),
    _durationsFloat.transpose()(0), _durationsFloat.transpose()(1), _durationsFloat.transpose()(2), _durationsFloat.transpose()(3));
...
VisionMPCLocomotion::VisionMPCLocomotion(float _dt, int _iterations_between_mpc, BZL_UserParameters* parameters) :
  ...
  _logger("VisionMPCLocomotion")
{
  _parameters = parameters;
  dtMPC = dt * iterationsBetweenMPC;
  QUADRUPED_INFO(_logger, "dt: %.3f iterations: %d, dtMPC: %.3f",
