...
class test_spi_command_t(object):
    __slots__ = ["is_test", "q_des_abad", "q_des_hip", "q_des_knee", "qd_des_abad", "qd_des_hip", "qd_des_knee", "kp_abad", "kp_hip", "kp_knee", "kd_abad", "kd_hip", "kd_knee", "tau_abad_ff", "tau_hip_ff", "tau_knee_ff", "flags"]
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(test_spi_command_t._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">i", self.is_test))
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != test_spi_command_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return test_spi_command_t._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = test_spi_command_t()
        self.is_test = struct.unpack(">i", buf.read(4))[0]
    ...
    def _get_hash_recursive(parents):
        if test_spi_command_t in parents: return 0
        tmphash = (0xba861c4af297c114) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if test_spi_command_t._packed_fingerprint is None:
            test_spi_command_t._packed_fingerprint = struct.pack(">Q", test_spi_command_t._get_hash_recursive([]))
        return test_spi_command_t._packed_fingerprint
